...
def purge_vm_data(conn, node, vm_uuid, refresh_snapshots=True):
    ...
    deleted["usage"] = _delete_count(
        conn,
        "DELETE FROM usage WHERE node=? AND vm_uuid=?",
        (node, vm_uuid),
    )
    deleted["node_stats"] = _delete_count(
        conn,
        "DELETE FROM node_stats WHERE node=? AND vm_uuid=?",
        (node, vm_uuid),
    )
    deleted["vm_perf_stats"] = _delete_count(
        conn,
        "DELETE FROM vm_perf_stats WHERE node=? AND vm_uuid=?",
        (node, vm_uuid),
    )
    deleted["vm_latest_metrics"] = _delete_count(
        conn,
        "DELETE FROM vm_latest_metrics WHERE node=? AND vm_uuid=?",
        (node, vm_uuid),
    )
    deleted["bandwidth_hourly"] = _delete_count(
        conn,
        "DELETE FROM bandwidth_hourly WHERE node=? AND vm_uuid=?",
        (node, vm_uuid),
    )
    deleted["bandwidth_daily"] = _delete_count(
        conn,
        "DELETE FROM bandwidth_daily WHERE node=? AND vm_uuid=?",
        (node, vm_uuid),
    )
    deleted["vm_node_presence"] = _delete_count(
        conn,
        "DELETE FROM vm_node_presence WHERE node=? AND vm_uuid=?",
        (node, vm_uuid),
    )
    deleted["vm_inventory"] = _delete_count(
        conn,
        "DELETE FROM vm_inventory WHERE node=? AND vm_uuid=?",
        (node, vm_uuid),
    )
    deleted["vm_migration_events"] = _delete_count(
        conn,
        """
        DELETE FROM vm_migration_events
        WHERE vm_uuid=?
          AND (old_node=? OR new_node=?)
        """,
        (vm_uuid, node, node),
    )
    ensure_disk_io_schema(conn)
    deleted["vm_disk_current"] = _delete_count(
        conn, "DELETE FROM vm_disk_current WHERE node=? AND vm_uuid=?", (node, vm_uuid)
    )

    _repair_vm_location_after_purge(conn, vm_uuid, node)
    if refresh_snapshots:
        _refresh_node_snapshot_vm_counts(conn, node)
    return deleted
