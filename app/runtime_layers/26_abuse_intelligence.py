...
def _v48126_ram_hit(cfg, metrics):
    ...
    if (
        ...
        ratios.append(cfg["ram_low_usable_percent"] / max(metrics["usable_percent"], 0.1))
...
def refresh_fast_current_state(conn, node, data_time, interval_seconds, interfaces, vms, node_host, inventory_complete=False):
    ...
    # Persist the Agent balloon fields before RAM evaluation. The cycle-v2
    # base writer does not know about unused/usable, so without this bridge the
    # Guest Used and Low Usable rules would see zeros in the current cache.
    for vm_item in vms or []:
        ...
        if not vm_uuid or vm_uuid == "-":
            continue
        ram_unused = max(0, safe_int(vm_item.get("ram_unused_kib"), 0))
        ram_usable = max(0, safe_int(vm_item.get("ram_usable_kib"), 0))
        conn.execute(
            "UPDATE vm_current_fast SET ram_unused_kib=?,ram_usable_kib=? WHERE node=? AND vm_uuid=?",
            (ram_unused, ram_usable, node, vm_uuid),
        )
        conn.execute(
    ...
    for row in current_rows:
        ...
        conn.execute("""
            UPDATE vm_abuse_state SET
              last_seen=?,is_abuse=?,abuse_since=?,abuse_flags=?,severity=?,
              network_rx_hit=?,network_tx_hit=?,network_rx_mbps_hit=?,network_tx_mbps_hit=?,
              cpu_streak_seconds=?,disk_streak_seconds=?,ram_streak_seconds=?,
              network_rx_mbps_streak_seconds=?,network_tx_mbps_streak_seconds=?,
              cpu_streak_cycles=?,disk_streak_cycles=?,ram_streak_cycles=?,
              network_rx_mbps_streak_cycles=?,network_tx_mbps_streak_cycles=?,
              rx_pps=?,tx_pps=?,rx_peak_pps=?,tx_peak_pps=?,seconds_over_rx_pps=?,seconds_over_tx_pps=?,
              rx_mbps=?,tx_mbps=?,cpu_full_percent=?,cpu_core_percent=?,vcpu_current=?,
              disk_read_bps=?,disk_write_bps=?,disk_read_iops=?,disk_write_iops=?,
              ram_current_kib=?,ram_rss_kib=?,ram_available_kib=?,ram_usable_kib=?,
              ram_rss_percent=?,ram_guest_used_percent=?,ram_usable_percent=?,
        ...
        """, (
            ...
            cpu_cycles * ABUSE_EVAL_CYCLE_SECONDS, disk_cycles * ABUSE_EVAL_CYCLE_SECONDS,
            ram_cycles * ABUSE_EVAL_CYCLE_SECONDS,
            rx_mbps_cycles * ABUSE_EVAL_CYCLE_SECONDS, tx_mbps_cycles * ABUSE_EVAL_CYCLE_SECONDS,
            cpu_cycles, disk_cycles, ram_cycles, rx_mbps_cycles, tx_mbps_cycles,
            safe_float(rx_pps, 0), safe_float(tx_pps, 0), safe_float(rx_peak_pps, 0), safe_float(tx_peak_pps, 0),
            safe_int(over_rx_pps, 0) if pps_synced else 0, safe_int(over_tx_pps, 0) if pps_synced else 0,
            safe_float(rx_mbps, 0), safe_float(tx_mbps, 0),
            safe_float(cpu_full, 0), safe_float(cpu_core, 0), safe_int(vcpu, 0),
            safe_float(disk_read, 0), safe_float(disk_write, 0), safe_float(disk_read_iops, 0), safe_float(disk_write_iops, 0),
            ram_metrics["current_kib"], ram_metrics["rss_kib"], ram_metrics["available_kib"], ram_metrics["usable_kib"],
            ram_metrics["rss_percent"], ram_metrics["guest_used_percent"], ram_metrics["usable_percent"],
