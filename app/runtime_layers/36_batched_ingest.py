...
def process_node_vm_presence(conn, node, seen_vm_locations, seen_ts, inventory_complete=False):
    """Set-based common path; rare migration transitions retain legacy logic."""
    incoming = []
    for vm_uuid, loc in (seen_vm_locations or {}).items():
        ...
        incoming.append({
            "vm_uuid": vm_uuid,
            "iface": str(loc.get("iface") or "-"),
            "bridge": str(loc.get("bridge") or "-"),
        })
    payload = json.dumps(incoming, separators=(",", ":"), ensure_ascii=False)

    if incoming:
        conn.execute("""
          WITH src AS (
            SELECT * FROM jsonb_to_recordset(?::jsonb)
              AS x(vm_uuid text, iface text, bridge text)
          )
        ...
        """, (payload, node, seen_ts, seen_ts, seen_ts))
        ...
        conn.execute("""
          ...
          SELECT ?,vm_uuid,?,?,iface,bridge,'active',NULL,NULL FROM src
        ...
        """, (payload, node, seen_ts, seen_ts))

    if inventory_complete:
        conn.execute("""
          WITH src AS (
            SELECT vm_uuid FROM jsonb_to_recordset(?::jsonb)
              AS x(vm_uuid text, iface text, bridge text)
          ), missing AS (
            UPDATE vm_node_presence p
             ...
             WHERE p.node=?
               ...
               AND NOT EXISTS (SELECT 1 FROM src WHERE src.vm_uuid=p.vm_uuid)
            RETURNING p.vm_uuid
          )
        ...
        """, (payload, seen_ts, seen_ts, node, node))

    if not incoming:
        return
    uuids = [row["vm_uuid"] for row in incoming]
    old_rows = conn.execute("""
      SELECT vm_uuid,node,last_seen,move_count
      FROM vm_location_latest
      WHERE vm_uuid=ANY(?)
    """, (uuids,)).fetchall()
    old_map = {str(row[0]): row for row in old_rows}
    new_rows = [row for row in incoming if row["vm_uuid"] not in old_map]
    same_rows = [row for row in incoming if row["vm_uuid"] in old_map and str(old_map[row["vm_uuid"]][1]) == node]
    moved_rows = [row for row in incoming if row["vm_uuid"] in old_map and str(old_map[row["vm_uuid"]][1]) != node]

    if new_rows:
        new_payload = json.dumps(new_rows, separators=(",", ":"), ensure_ascii=False)
        conn.execute("""
          WITH src AS (
            SELECT * FROM jsonb_to_recordset(?::jsonb)
              AS x(vm_uuid text, iface text, bridge text)
          ...
          SELECT vm_uuid,?,?,?,NULL,NULL,0,iface,bridge,'ok','' FROM src
          ON CONFLICT(vm_uuid) DO NOTHING
        """, (new_payload, node, seen_ts, seen_ts))

    if same_rows:
        same_payload = json.dumps(same_rows, separators=(",", ":"), ensure_ascii=False)
        conn.execute("""
          WITH src AS (
            SELECT * FROM jsonb_to_recordset(?::jsonb)
              AS x(vm_uuid text, iface text, bridge text)
          )
          UPDATE vm_location_latest l
             SET last_seen=GREATEST(l.last_seen,?),last_iface=src.iface,last_bridge=src.bridge,
                 alert_level='ok',alert_flags=''
            FROM src
           WHERE l.vm_uuid=src.vm_uuid AND l.node=?
        """, (same_payload, seen_ts, node))

    # Cross-node transitions are rare and retain the proven confirmation/event path.
    for row in moved_rows:
        _v5050_legacy_location_transition(
            conn, row["vm_uuid"], node, seen_ts, row["iface"], row["bridge"]
        )
