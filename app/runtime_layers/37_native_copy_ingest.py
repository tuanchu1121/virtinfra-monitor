...
def _v5052_iface_rows(node, data_time, bucket, interval_seconds, interfaces):
    ...
    rows = []
    _slot_start, hour_start, day_start, _slot_no = _r2211_slot_coordinates(data_time)
        ...
        conn.execute("""
          WITH grouped AS (
            SELECT hour_start,node,vm_uuid,bridge,
                   ...
                   LEAST(11,GREATEST(0,((MAX(bucket)-300-hour_start)/300)::integer)) AS slot_no
              FROM pg_temp.vi5052_iface_stage
          ...
          INSERT INTO vm_consumption_hourly(
            ...
            rx_5m_slots,tx_5m_slots,sample_5m_mask,slot_5m_version
          )
          SELECT hour_start,node,vm_uuid,bridge,rx_bytes,tx_bytes,rx_packets,tx_packets,
                 rx_drops,tx_drops,rx_errors,tx_errors,sample_count,last_push,
                 ARRAY[CASE WHEN slot_no=0 THEN rx_bytes ELSE 0 END,CASE WHEN slot_no=1 THEN rx_bytes ELSE 0 END,CASE WHEN slot_no=2 THEN rx_bytes ELSE 0 END,CASE WHEN slot_no=3 THEN rx_bytes ELSE 0 END,CASE WHEN slot_no=4 THEN rx_bytes ELSE 0 END,CASE WHEN slot_no=5 THEN rx_bytes ELSE 0 END,CASE WHEN slot_no=6 THEN rx_bytes ELSE 0 END,CASE WHEN slot_no=7 THEN rx_bytes ELSE 0 END,CASE WHEN slot_no=8 THEN rx_bytes ELSE 0 END,CASE WHEN slot_no=9 THEN rx_bytes ELSE 0 END,CASE WHEN slot_no=10 THEN rx_bytes ELSE 0 END,CASE WHEN slot_no=11 THEN rx_bytes ELSE 0 END]::bigint[],ARRAY[CASE WHEN slot_no=0 THEN tx_bytes ELSE 0 END,CASE WHEN slot_no=1 THEN tx_bytes ELSE 0 END,CASE WHEN slot_no=2 THEN tx_bytes ELSE 0 END,CASE WHEN slot_no=3 THEN tx_bytes ELSE 0 END,CASE WHEN slot_no=4 THEN tx_bytes ELSE 0 END,CASE WHEN slot_no=5 THEN tx_bytes ELSE 0 END,CASE WHEN slot_no=6 THEN tx_bytes ELSE 0 END,CASE WHEN slot_no=7 THEN tx_bytes ELSE 0 END,CASE WHEN slot_no=8 THEN tx_bytes ELSE 0 END,CASE WHEN slot_no=9 THEN tx_bytes ELSE 0 END,CASE WHEN slot_no=10 THEN tx_bytes ELSE 0 END,CASE WHEN slot_no=11 THEN tx_bytes ELSE 0 END]::bigint[],(1 << slot_no)::integer,2
            FROM grouped
          ON CONFLICT(hour_start,node,vm_uuid,bridge) DO UPDATE SET
            ...
            rx_5m_slots=CASE WHEN COALESCE(vm_consumption_hourly.slot_5m_version,1)<2 THEN excluded.rx_5m_slots ELSE ARRAY[COALESCE(vm_consumption_hourly.rx_5m_slots[1],0)+COALESCE(excluded.rx_5m_slots[1],0),COALESCE(vm_consumption_hourly.rx_5m_slots[2],0)+COALESCE(excluded.rx_5m_slots[2],0),COALESCE(vm_consumption_hourly.rx_5m_slots[3],0)+COALESCE(excluded.rx_5m_slots[3],0),COALESCE(vm_consumption_hourly.rx_5m_slots[4],0)+COALESCE(excluded.rx_5m_slots[4],0),COALESCE(vm_consumption_hourly.rx_5m_slots[5],0)+COALESCE(excluded.rx_5m_slots[5],0),COALESCE(vm_consumption_hourly.rx_5m_slots[6],0)+COALESCE(excluded.rx_5m_slots[6],0),COALESCE(vm_consumption_hourly.rx_5m_slots[7],0)+COALESCE(excluded.rx_5m_slots[7],0),COALESCE(vm_consumption_hourly.rx_5m_slots[8],0)+COALESCE(excluded.rx_5m_slots[8],0),COALESCE(vm_consumption_hourly.rx_5m_slots[9],0)+COALESCE(excluded.rx_5m_slots[9],0),COALESCE(vm_consumption_hourly.rx_5m_slots[10],0)+COALESCE(excluded.rx_5m_slots[10],0),COALESCE(vm_consumption_hourly.rx_5m_slots[11],0)+COALESCE(excluded.rx_5m_slots[11],0),COALESCE(vm_consumption_hourly.rx_5m_slots[12],0)+COALESCE(excluded.rx_5m_slots[12],0)]::bigint[] END,
            tx_5m_slots=CASE WHEN COALESCE(vm_consumption_hourly.slot_5m_version,1)<2 THEN excluded.tx_5m_slots ELSE ARRAY[COALESCE(vm_consumption_hourly.tx_5m_slots[1],0)+COALESCE(excluded.tx_5m_slots[1],0),COALESCE(vm_consumption_hourly.tx_5m_slots[2],0)+COALESCE(excluded.tx_5m_slots[2],0),COALESCE(vm_consumption_hourly.tx_5m_slots[3],0)+COALESCE(excluded.tx_5m_slots[3],0),COALESCE(vm_consumption_hourly.tx_5m_slots[4],0)+COALESCE(excluded.tx_5m_slots[4],0),COALESCE(vm_consumption_hourly.tx_5m_slots[5],0)+COALESCE(excluded.tx_5m_slots[5],0),COALESCE(vm_consumption_hourly.tx_5m_slots[6],0)+COALESCE(excluded.tx_5m_slots[6],0),COALESCE(vm_consumption_hourly.tx_5m_slots[7],0)+COALESCE(excluded.tx_5m_slots[7],0),COALESCE(vm_consumption_hourly.tx_5m_slots[8],0)+COALESCE(excluded.tx_5m_slots[8],0),COALESCE(vm_consumption_hourly.tx_5m_slots[9],0)+COALESCE(excluded.tx_5m_slots[9],0),COALESCE(vm_consumption_hourly.tx_5m_slots[10],0)+COALESCE(excluded.tx_5m_slots[10],0),COALESCE(vm_consumption_hourly.tx_5m_slots[11],0)+COALESCE(excluded.tx_5m_slots[11],0),COALESCE(vm_consumption_hourly.tx_5m_slots[12],0)+COALESCE(excluded.tx_5m_slots[12],0)]::bigint[] END,
            sample_5m_mask=CASE WHEN COALESCE(vm_consumption_hourly.slot_5m_version,1)<2 THEN excluded.sample_5m_mask ELSE COALESCE(vm_consumption_hourly.sample_5m_mask,0)|excluded.sample_5m_mask END,
            slot_5m_version=2
        """)
        conn.execute("""
          WITH grouped AS (
