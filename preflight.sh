...
[[ "$(cat VERSION)" == "50.5.9-prod-r22.12-vm-consumption-shared-snapshot" ]] || fail "VERSION mismatch"
[[ -f app/app.py && -f app/runtime_loader.py \
   ...
   && -f postgres/sql/017_vm_consumption_5m_slots.sql && -f postgres/sql/018_vm_consumption_slot_boundary_semantics.sql && -f postgres/sql/019_vm_consumption_shared_snapshot.sql && -f app/runtime_layers/47_vm_5m_slot_rolling_window.py && -f app/runtime_layers/48_vm_consumption_shared_snapshot.py && -f app/vm_consumption_snapshot.py && -f app/node_groups.py \
   ...
   && -f tests/test_r22_7_vm_consumption_rollup_only.py && -f tests/test_r22_10_vm_5m_slot_rolling.py ]] \
...
log "Validate r22.10 packed five-minute VM rolling windows"
run_pytest -q tests/test_r22_10_vm_5m_slot_rolling.py
