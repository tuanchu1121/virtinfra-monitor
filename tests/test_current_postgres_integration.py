...
os.environ.update({
    ...
    "BW_MONITOR_TOKEN": "current-integration-token",
    "BW_ADMIN_USERNAME": "admin",
    "BW_ADMIN_PASSWORD_HASH": "",
    "BW_ADMIN_SECRET_KEY": "current-integration-secret-key",
    "BW_REDIS_ENABLED": "0",
...
spec = importlib.util.spec_from_file_location("bw_monitor_current_app", ROOT / "app/app.py")
...
response = client.post("/push", json=payload, headers={"X-Token": "current-integration-token"})
assert response.status_code == 200, response.get_data(as_text=True)
assert response.get_json().get("ok") is True
response2 = client.post("/push", json=payload, headers={"X-Token": "current-integration-token"})
...
def post_concurrent_sample():
    ...
    result = thread_client.post(
        ...
        headers={"X-Token": "current-integration-token"},
    )
...
partial_response = client.post("/push", json=partial_payload, headers={"X-Token": "current-integration-token"})
...
newer_response = client.post("/push", json=newer_payload, headers={"X-Token": "current-integration-token"})
...
older_response = client.post("/push", json=older_payload, headers={"X-Token": "current-integration-token"})
...
future_response = client.post("/push", json=future_payload, headers={"X-Token": "current-integration-token"})
...
bw_response = client.post("/push/bandwidth-consumption", json=bw_payload, headers={"X-Token": "current-integration-token"})
...
bw_retry = client.post("/push/bandwidth-consumption", json=bw_payload, headers={"X-Token": "current-integration-token"})
...
print("PASS: current app PostgreSQL/Timescale integration, chart/raw writes, pages, duplicate push and exact UUID purge")
