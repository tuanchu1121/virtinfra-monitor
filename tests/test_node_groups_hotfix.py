...
EXPECTED_RELEASE = "50.5.9-prod-r22.8-vm-consumption-exact-window-sort-alignment"
...
def test_existing_postgresql_migrations_are_byte_identical():
    ...
    for path in sorted((ROOT / "postgres/sql").glob("0[0-1][0-9]_*.sql")):
        if path.name in {"011_node_groups.sql", "012_node_groups_r6_safety.sql", "013_maintenance_queue_boolean.sql", "014_node_vm_consumption_rollups.sql", "015_consumption_ingest_preaggregation.sql", "016_configuration_backup_nuclear.sql"}:
            continue
