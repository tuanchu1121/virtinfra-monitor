...
EXPECTED_RELEASE = "50.5.9-prod-r5-node-groups-hotfix-additive"
...
def test_hotfix_is_appended_after_untouched_baseline_app():
    data = (ROOT / "app/app.py").read_bytes()
    marker = (
        b"\n# ---------------------------------------------------------------------------\n"
        b"# VirtInfra Monitor 50.5.9 prod-r5 - additive Node Groups hotfix\n"
    )
...
def test_existing_postgresql_migrations_are_byte_identical():
    ...
    for path in sorted((ROOT / "postgres/sql").glob("0[0-1][0-9]_*.sql")):
        if path.name == "011_node_groups.sql":
            continue
...
def test_runtime_role_crud_assignment_inheritance_and_filters(runtime_result):
    ...
    expected = {
        ...
        "admin_permission_boundary", "admin_user_management", "viewer_read_only",
        "route_contract",
    }
    assert {key for key, value in result.items() if value == "PASS"} == expected
    assert result["route_count"] == 79


def test_route_map_preserves_baseline_and_only_adds_four_posts(runtime_result):
...
def test_ui_regression_contract_passes():
    ...
    assert set(report) == {
        "admin-overview", "admin-nodes", "admin-vms", "dashboard", "top-vm",
        "node-health", "storage-io", "consumption", "vm-abuse",
    }
...
def test_browser_ui_regression_passes_desktop_tablet_and_mobile():
    ...
    assert set(report) == {
        "admin-overview", "admin-nodes", "admin-vms", "admin-node-groups",
        "dashboard", "top-vm", "node-health", "storage-io", "consumption", "vm-abuse",
    }
    assert all(
...
def test_local_flag_icons_are_vendored_without_runtime_network_dependency():
    root = ROOT / "app/static/vendor/flag-icons"
    assert (root / "LICENSE").is_file()
    assert "MIT License" in (root / "LICENSE").read_text(encoding="utf-8")
    assert (root / "SOURCE.md").is_file()
    flags = list((root / "flags/4x3").glob("*.svg"))
    assert len(flags) >= 240
    for required in ("vn.svg", "jp.svg", "sg.svg"):
        assert (root / "flags/4x3" / required).is_file()
    css = (root / "node-groups.css").read_text(encoding="utf-8")
    assert "https://" not in css and "http://" not in css and "//cdn" not in css
    assert "width:16px" in css and "height:12px" in css
    assert "(_FLAG_DIR / f\"{code}.svg\").is_file()" in MODULE.read_text(encoding="utf-8")
...
def test_installer_copies_and_applies_additive_files():
    ...
    assert 'install -m 0644 "$APP_SRC/static/vendor/flag-icons/SOURCE.md"' in installer
    assert 'find "$APP_SRC/static/vendor/flag-icons/flags/4x3"' in installer
