...
EXPECTED_VERSION = "50.5.9-prod-r9-safe-runtime-history-prune"
...
def test_release_identity_and_cleanup_reports() -> None:
    assert (ROOT / "VERSION").read_text(encoding="utf-8").strip() == EXPECTED_VERSION
    assert (ROOT / "CLEANUP_REPORT_R9.md").is_file()
    assert (ROOT / "VALIDATION_REPORT_R9.md").is_file()
