...
EXPECTED_VERSION = "50.5.9-prod-r22.5-configuration-backup-nuclear-hardening"
...
def test_runtime_layers_are_hash_pinned_and_contiguous() -> None:
    ...
    for item in _manifest():
        ...
        total += line_count
    assert total == sum(len((LAYER_DIR / str(item["file"])).read_text(encoding="utf-8").splitlines()) for item in _manifest())
