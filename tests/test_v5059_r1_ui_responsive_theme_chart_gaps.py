...
def test_release_marker():
    assert (ROOT / "VERSION").read_text(encoding="utf-8").strip() == "50.6.0-prod-r1-node-groups-country-flags"
    assert 'V5059R2_RELEASE = "50.6.0-prod-r1-node-groups-country-flags"' in APP
...
def test_snapshot_tables_are_collapsed_by_default():
    ...
    assert 'aria-controls="real-snapshot-samples-panel"' in APP
    assert 'aria-controls="retained-network-snapshots-panel"' in APP
    assert "v5059r1-snapshot-accessibility" in APP
    assert "setAttribute('aria-expanded', details.open ? 'true' : 'false')" in APP
