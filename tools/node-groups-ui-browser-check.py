...
def main() -> int:
        ...
        for before_file in sorted(args.before.glob("*.html")):
            ...
            for viewport_name, viewport in VIEWPORTS.items():
                ...
                for path in (before_file, after_file):
                    ...
                    if path == after_file:
                        feature_css = (Path(__file__).resolve().parents[1] / "app/static/vendor/flag-icons/node-groups.css").read_text(encoding="utf-8")
                ...
                checks["no_new_document_overflow"] = (
                    ...
                    and after["document"]["scrollWidth"] <= before["document"]["scrollWidth"] + 1
                )
                ...
                checks["card_geometry_preserved"] = checks["card_count_identical"] and all(
                    close(left["rect"][key], right["rect"][key])
                    for left, right in zip(before["cards"], after["cards"])
                    for key in ("x", "width")
                )
    ...
    print(f"Browser UI regression {'passed' if not failures else 'failed'} for {len(result)} pages across {len(VIEWPORTS)} viewports")
