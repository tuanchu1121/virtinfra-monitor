...
def main() -> int:
    ...
    for before_file in sorted(args.before.glob("*.html")):
        ...
        old_headers = texts(before.find_all("th"))
        new_headers = texts(after.find_all("th"))
        ...
        old_nav = texts(before.select("nav a"))
        new_nav = texts(after.select("nav a"))
        item["old_navigation_preserved"] = is_subsequence(old_nav, new_nav)
        if name.startswith("admin-"):
            try:
                item["node_groups_nav_position"] = new_nav.index("Node Groups") + 1 == new_nav.index("Nodes")
            except ValueError:
