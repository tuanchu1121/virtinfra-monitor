...
grep -q 'RELEASE="50.6.0-prod-r2-node-groups-update-detection-fix"' "$I" || fail "release marker missing"
...
grep -q 'static/flags/4x3' "$I" || fail "local SVG flag assets are not installed"
grep -q 'THIRD_PARTY_LICENSES/flag-icons-LICENSE.txt' "$I" || fail "flag-icons MIT license is not installed"
